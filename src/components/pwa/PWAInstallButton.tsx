      ...
      {showIOSGuide && (
        ...
        >
          ...
          >
            ...
            {!isIOS && (
              <div className="mb-4 p-3 rounded-xl bg-teal-950/40 border border-teal-500/30 flex flex-col gap-2">
                <span className="text-xs font-semibold text-teal-300">Prefere instalar via APK direto?</span>
                <a
                  href="/connect-algodoal.apk"
                  download="connect-algodoal.apk"
                  id="pwa-guide-download-apk-btn"
                  className="flex items-center justify-center gap-2 py-2 px-3 rounded-lg bg-teal-500 hover:bg-teal-400 text-slate-950 font-bold text-xs shadow transition text-center"
                >
                  <Download className="w-3.5 h-3.5 shrink-0" />
                  <span>Baixar APK Android (.apk)</span>
                </a>
              </div>
            )}
            ...
            {isIOS ? (
              ...
              <div className="space-y-3 text-xs text-slate-300">
                <p className="font-medium text-teal-300">Instalar pelo Navegador (Sem ocupar espaço):</p>
                <div className="flex items-start gap-2 bg-slate-800/80 p-2.5 rounded-lg border border-slate-700">
            ...
            <button
              ...
              onClick={() => setShowIOSGuide(false)}
              className="mt-4 w-full py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs border border-slate-600 transition cursor-pointer"
            >
              Fechar
            </button>
          </div>
