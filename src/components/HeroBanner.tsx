...
import { IslandImages } from '../assets/images/index.ts';
...
export const HeroBanner: React.FC<HeroBannerProps> = ({
  ...
  const handleAdCardClick = (category: ServiceCategory, ad?: Advertisement) => {
    if (ad?.id) {
      api.recordAdMetric(ad.id, 'click');
    }
    onSelectCategory(category);
    
    // Smooth scroll down to the category items list
    const servicesSection = document.getElementById('secao-servicos-categoria');
    if (servicesSection) {
  ...
  return (
    <div className="bg-[#f8fafc] py-4 sm:py-6 border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        ...
        {/* Main Grid: Left Official Island Art Banner + Right 4 Showcase Ad Cards */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 items-stretch">
          
          {/* ========================================================= */}
          {/* LEFT SIDE: ALGODOAL CONNECT OFFICIAL HERO BANNER (FIXO)   */}
          {/* ========================================================= */}
          <div className="lg:col-span-8 relative rounded-3xl overflow-hidden shadow-xl border border-sky-300/60 bg-gradient-to-br from-[#024976] via-[#075985] to-[#0c4a6e] min-h-[440px] sm:min-h-[500px] flex flex-col justify-between text-white p-5 sm:p-8">
            ...
            {/* Background Tropical Sunset Landscape from Real Algodoal Image */}
            <div 
            ...
            <div className="absolute top-4 right-1/4 sm:right-1/3 pointer-events-none opacity-85">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="space-y-1">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-400/95 text-slate-950 text-xs font-black uppercase tracking-wider mb-2 shadow-md">
                ...
                <p className="text-xs sm:text-sm font-bold text-sky-100 max-w-lg drop-shadow-xs">
                  O guia completo e catálogo oficial de serviços, charretes, rabetas, pousadas e culinária da Ilha de Algodoal.
                </p>
              </div>
          ...
          {/* ========================================================= */}
          {/* RIGHT SIDE: 4 SHOWCASE ADS (ANÚNCIOS DIRETOS DOS SERVIÇOS)*/}
          {/* ========================================================= */}
          <div className="lg:col-span-4 flex flex-col gap-3 justify-between">
            ...
            <div 
              id="hero-banner-1"
              onClick={() => handleAdCardClick('transporte', ad1)}
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <h4 className="text-base sm:text-lg font-black text-slate-900 tracking-tight font-heading group-hover:text-amber-600 transition">
                  {ad1?.title ? (ad1.title.length > 22 ? ad1.title.slice(0, 22) + '...' : ad1.title) : 'Charretes Credenciadas'}
                </h4>
              </div>

              <div className="my-1.5">
                <h5 className="text-xs sm:text-sm font-black text-slate-900">
                  {ad1?.business_name || 'Associação dos Condutores de Charrete de Maiandeua'}
                </h5>
                <p className="text-[11px] text-slate-600 leading-tight mt-0.5 line-clamp-2">
                  {ad1?.tagline || ad1?.description || 'Desembarque com tranquilidade e transporte sua bagagem com preço tabelado'}
                </p>
              </div>
              ...
              <div className="flex items-center justify-between pt-1 border-t border-slate-100">
                ...
                <span className="inline-flex items-center gap-1 text-xs font-bold text-amber-800 group-hover:translate-x-1 transition">
            ...
            <div 
              id="hero-banner-2"
              onClick={() => handleAdCardClick('alimentacao', ad2)}
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <h4 className="text-base sm:text-lg font-black text-slate-900 tracking-tight font-heading group-hover:text-rose-800 transition">
                  {ad2?.title ? (ad2.title.length > 22 ? ad2.title.slice(0, 22) + '...' : ad2.title) : 'Peixada & Caldeirada'}
                </h4>
              </div>

              <div className="my-1.5">
                <h5 className="text-xs sm:text-sm font-black text-slate-900">
                  {ad2?.business_name || 'Restaurante O Marujo'}
                </h5>
                <p className="text-[11px] text-slate-600 leading-tight mt-0.5 line-clamp-2">
                  {ad2?.tagline || ad2?.description || 'O melhor peixe frito com açaí e frutos do mar frescos nos quiosques'}
                </p>
              </div>

              <div className="flex items-center justify-between pt-1 border-t border-slate-100">
                <span className="text-xs font-black text-rose-900">
                  {ad2?.badge || 'Mais Recomendado'}
                </span>
                <span className="inline-flex items-center gap-1 text-xs font-bold text-rose-900 group-hover:translate-x-1 transition">
            ...
            <div 
              id="hero-banner-3"
              onClick={() => handleAdCardClick('compras', ad3)}
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <h4 className="text-base sm:text-lg font-black text-slate-900 tracking-tight font-heading group-hover:text-lime-700 transition">
                  {ad3?.title ? (ad3.title.length > 22 ? ad3.title.slice(0, 22) + '...' : ad3.title) : 'Depósito Ilha Bela'}
                </h4>
              </div>

              <div className="my-1.5">
                <h5 className="text-xs sm:text-sm font-black text-slate-900">
                  {ad3?.business_name || 'Depósito Ilha Bela - Água 20L & Gelo'}
                </h5>
                <p className="text-[11px] text-slate-600 leading-tight mt-0.5 line-clamp-2">
                  {ad3?.tagline || ad3?.description || 'Entrega rápida de água mineral, gelo e carvão na sua pousada'}
                </p>
              </div>

              <div className="flex items-center justify-between pt-1 border-t border-slate-100">
                <span className="text-xs font-black text-lime-900">
                  {ad3?.price_starting ? `Garrafão: R$ ${ad3.price_starting.toFixed(2)}` : 'Garrafão: R$ 14,00'}
                </span>
                <span className="inline-flex items-center gap-1 text-xs font-bold text-lime-800 group-hover:translate-x-1 transition">
            ...
            <div 
              id="hero-banner-4"
              onClick={() => handleAdCardClick('passeios', ad4)}
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-bold text-slate-400">Roteiros Náuticos</span>
                </div>
                <h4 className="text-base sm:text-lg font-black text-slate-900 tracking-tight font-heading group-hover:text-sky-600 transition">
                  {ad4?.title ? (ad4.title.length > 22 ? ad4.title.slice(0, 22) + '...' : ad4.title) : 'Rabetas & Lago'}
                </h4>
              </div>

              <div className="my-1.5">
                <h5 className="text-xs sm:text-sm font-black text-slate-900">
                  {ad4?.business_name || 'Roteiros de Rabeta & Lago da Princesa'}
                </h5>
                <p className="text-[11px] text-slate-600 leading-tight mt-0.5 line-clamp-2">
                  {ad4?.tagline || ad4?.description || 'Navegue pelo Furo Velho, Lago da Princesa, dunas e Fortalezinha'}
                </p>
              </div>
              ...
              <div className="flex items-center justify-between pt-1 border-t border-slate-100">
                ...
                <span className="inline-flex items-center gap-1 text-xs font-bold text-sky-800 group-hover:translate-x-1 transition">
